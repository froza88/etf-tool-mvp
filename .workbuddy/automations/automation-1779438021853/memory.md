...
| 步骤 | 状态 | 说明 |
|------|------|------|
| 1. pipeline.py --push --no-wind | 成功 | sync 1511(+1) → enrich 62(+3) → calc 200/368 → build 1511 → git push ✅，349版本 |
| 2. wequote_daily.py | 成功 | 03:01-03:15，13m50s，152批次quote+152批次etf |
| 3. git commit + push | 成功 | e00e63a，2432 增/1976 删 |
| 4. PA 同步 curl | 失败(404) | 连续第 18 天 |

### 数据更新
- Pipeline: 1511 ETF，版本清单 349 个版本，git push 成功（2 次 deploy commit）
- WeStock: quote 更新 1511 个字段，etf 更新 1216 个字段，总计 2727 字段
- 字段覆盖率: custodian 1498/1511 (99.1%), fee_rate 1394/1511 (92.3%), benchmark 1491/1511 (98.7%), premium_discount 145/1511 (9.6%)
- Git: 2432 行新增，1976 行删除

### 问题
- PA 同步返回 404（连续第 18 天），PythonAnywhere 疑似下线
...
| 步骤 | 状态 | 说明 |
|------|------|------|
| 1. pipeline.py --push --no-wind | 成功 | 02:59-03:00，~1m50s，2次deploy commit，332版本 |
| 2. wequote_daily.py | 成功 | 03:02-03:17，14m44s，一次通过 |
| 3. git commit + push | 成功 | 90c3500，2441 增/1988 删 |
| 4. PA 同步 curl | 失败(404) | 连续第 17 天 |

### 数据更新
- Pipeline: 1510 ETF，版本清单 332 个版本，git push 成功（2 次 deploy commit）
- WeStock: quote 更新 1510 个字段，etf 更新 1208 个字段，总计 2718 字段
- 字段覆盖率: custodian 1497/1510 (99.1%), fee_rate 1394/1510 (92.3%), benchmark 1489/1510 (98.6%), premium_discount 145/1510 (9.6%)
- Git: 2441 行新增，1988 行删除

### 问题
- PA 同步返回 404（连续第 17 天），PythonAnywhere 疑似下线
- logs/ 目录无当天日志文件（pipeline/wequote 写入 stdout，非 logs/）
...
| 步骤 | 状态 | 说明 |
|------|------|------|
| 1. pipeline.py --push --no-wind | 成功 | 02:57-02:58，~1m50s，2次deploy commit，325版本 |
| 2. wequote_daily.py | 成功 | 02:59-03:14，14m38s，一次通过 |
| 3. git commit + push | 成功 | 942e42c，2429 增/1976 删 |
| 4. PA 同步 curl | 失败(404) | 连续第 16 天 |

### 数据更新
- Pipeline: 1507 ETF，版本清单 325 个版本，git push 成功（2 次 deploy commit）
- WeStock: quote 更新 1507 个字段，etf 更新 1208 个字段，总计 2715 字段
- 字段覆盖率: custodian 1496/1507 (99.3%), fee_rate 1394/1507 (92.5%), benchmark 1487/1507 (98.7%), premium_discount 145/1507 (9.6%)
- 亮点: fee_rate 从昨日 59.8% 大幅提升至 92.5%（+32.7pp）
- Git: 2429 行新增，1976 行删除

### 问题
- PA 同步返回 404（连续第 16 天），PythonAnywhere 疑似下线
- logs/ 目录无当天日志文件（pipeline/wequote 写入 stdout，非 logs/）
